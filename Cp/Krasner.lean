...
class NonarchimedeanField (K : Type _) extends NormedField K, NonarchimedeanRing K
...
theorem Krasner (K : Type _) [NonarchimedeanField K] {Γ₀ : Type _} [LinearOrderedCommGroupWithZero Γ₀] [Valued K Γ₀] (L : Type _) [Field L] [vL : Valued L Γ₀] [i : ValuedFieldExtension K L] (f : Polynomial K) (hirr : Irreducible f) (hsp : Polynomial.Splits i.toRingHom f) : ∀ α : L, ∀ β : L, f.eval₂ i.toRingHom (α) = 0 → (∀ α' : L, f.eval₂ i.toRingHom (α') = 0 → α' ≠ α → vL.v (α - β) < vL.v (α - α')) → (K⟮α⟯ ≤ K⟮β⟯) := sorry